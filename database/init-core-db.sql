...
SELECT 
    ...
    p.phone_number,
    CASE 
        WHEN EXTRACT(YEAR FROM AGE(p.date_of_birth)) < 18 THEN 'Child'
        WHEN EXTRACT(YEAR FROM AGE(p.date_of_birth)) < 65 THEN 'Adult'
        ELSE 'Senior'
    END as age_category,
...
SELECT 
    ...
    d.experience_years,
    d.consultation_fee,
    d.available_for_consultation,
    CASE 
        WHEN d.experience_years >= 10 THEN 'Senior'
        WHEN d.experience_years >= 5 THEN 'Mid-level'
        ELSE 'Junior'
    END as seniority_level
FROM doctors d
WHERE d.deleted = false AND d.available_for_consultation = true;
